...
public:
    ...
    void InitVulkan()
    {
        ...
        m_cmdPool       = CreateCommandPool(device, m_context.queueFamilyIdx());
        m_cmdBuffers    = AllocateCommandBuffers(device, m_cmdPool, m_swapchain->images().size());

        // Some sync object
        m_imageFence       = CreateFence(device);
